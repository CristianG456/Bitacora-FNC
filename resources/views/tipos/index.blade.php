                ...
                <tr>
                    <td>
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                <h3 class="font-bold text-sm text-gray-900">{{ $tipo->nombre }}</h3>
                                @if($tipo->descripcion)
                                <p class="text-[12px] text-gray-500 mt-0.5">{{ $tipo->descripcion }}</p>
                                @endif
                            </div>
                        </div>
                    </td>
...
<div id="modal-crear-tipo" class="fixed inset-0 z-50 hidden bg-gray-900 bg-opacity-50 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden animate-fadeInScale">
        ...
        <form action="{{ route('tipos.store') }}" method="POST" class="p-5">
            @csrf
            <div class="space-y-4">
                <div>
