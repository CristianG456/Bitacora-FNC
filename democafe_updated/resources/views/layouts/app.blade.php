...
<div class="main-wrapper">
        ...
        <div class="header-right">
            ...
            <div class="relative" id="notif-container">
                <div class="notif-bell cursor-pointer" title="Notificaciones" id="notif-btn" onclick="toggleNotificaciones()">
                    ...
                    <span class="notif-badge" id="notif-badge-count" style="display: {{ $sinLeer > 0 ? 'inline-block' : 'none' }};">
                        {{ $sinLeer > 9 ? '9+' : $sinLeer }}
                    </span>
                </div>
    ...
    <nav class="bottom-nav">
        ...
        <button type="button" class="bottom-nav-item" onclick="toggleMobileDrawer()">
            <i data-lucide="menu" style="width:22px;height:22px;"></i>
            Más
        </button>
    </nav>

    {{-- Mobile Drawer (Menú "Más") --}}
    <div id="mobile-drawer" class="mobile-drawer">
        ...
        <div class="mobile-drawer-content">
            ...
            <a href="{{ route('tipos.index') }}" class="drawer-nav-item {{ request()->routeIs('tipos.*') ? 'active' : '' }}">
                <i data-lucide="file-text" style="width:20px;height:20px;"></i> Tipos de Documento
            </a>
            ...
            <a href="{{ route('historial.index') }}" class="drawer-nav-item {{ request()->routeIs('historial.*') ? 'active' : '' }}">
                <i data-lucide="clock" style="width:20px;height:20px;"></i> Historial Global
            </a>
            @endif
            ...
            <a href="{{ route('respaldos.index') }}" class="drawer-nav-item {{ request()->routeIs('respaldos.*') ? 'active' : '' }}">
                <i data-lucide="database" style="width:20px;height:20px;"></i> Respaldos
            </a>
            @endif
        </div>
    </div>
...
<script>
    ...
    function toggleNotificaciones() {
        ...
        if (!dropdown.classList.contains('hidden')) {
            ...
            // Automáticamente ocultar el badge y ponerlo a 0 visualmente
            const badge = document.getElementById('notif-badge-count');
            if (badge && badge.style.display !== 'none') {
                badge.style.display = 'none';
                badge.innerText = '0';
                
                // Llamar silenciosamente a marcar leídas en el servidor
                fetch('{{ route("notificaciones.marcar_leidas") }}', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                        'Accept': 'application/json'
                    }
                }).catch(e => console.log('Error marcando notificaciones:', e));
            }
        }
    }
